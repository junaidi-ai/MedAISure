...
def test_load_local_model(model_runner, mock_model, tmp_path):
    ...
    with patch("importlib.import_module") as mock_import:
        ...
        # and an empty dict for kwargs
        args, kwargs = mock_module.load_model.call_args
        # Check path is in the args
        assert str(model_path) in str(args[0])
        assert kwargs == {}
...
def test_run_model(model_runner, mock_model):
    ...
    inputs = [
        ...
        {
            "premise": "The patient has no symptoms.",
            "hypothesis": "The patient is healthy.",
        },
    ]
